...
if platform == 'darwin':
    ...
    AVR_BIN_PREFIX = 'avr-'
    SKETCHBOOK_HOME_DEFAULT = os.path.realpath('/home/mediocregopher/src/arduino/')

ARDUINO_BOARD_DEFAULT = os.environ.get('ARDUINO_BOARD', 'uno')
...
ARDUINO_LIBS = [pathJoin(ARDUINO_HOME, 'libraries'),pathJoin(SKETCHBOOK_HOME_DEFAULT, 'libraries')]
print("Library dirs: {0}".format(ARDUINO_LIBS))
if EXTRA_LIB:
...
print("{0}{1}".format(TARGET,FILE_EXTENSION))
...
envArduino = Environment(CC = AVR_BIN_PREFIX+'gcc', CXX = AVR_BIN_PREFIX+'g++', AS=AVR_BIN_PREFIX+'gcc',
    ...
    CFLAGS = cFlags+['-std=gnu99'], CCFLAGS = cFlags, ASFLAGS=['-mmcu=%s'%MCU],
    TOOLS = ['gcc','g++', 'as'])
